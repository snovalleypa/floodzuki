...
// We therefore pick the key by platform and never fall back across platforms
//  — a web key would 403 on native and vice-versa. Both come from app.config.ts
//  `extra` (MAP_TILER_KEY / MAP_TILER_KEY_NATIVE). When the platform's key is
// absent (e.g. local dev), the satellite base layer is not offered.
