...
`npm run typecheck` runs the full TypeScript check. `npm run validate` runs the combined lint, typecheck, and Jest gate. `npm test` remains Windows-safe and runs Jest in UTC by default.
